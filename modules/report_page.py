...
def format_report_table(
    ...
    # 4. Re-order kolom (M-to-M diletakkan di antara col_curr dan col_cum_prev)
    ordered_columns = [
        col_prev,
        col_curr,
        'M-to-M (%)',
...
def show_report_page():
    ...
    with c1:
        prov = st.selectbox('Provinsi', list(PEMETAAN_WILAYAH.keys()))
    with c2:
        thn = st.selectbox('Tahun', ['2024', '2025', '2026'], index=1)
    with c3:
        bln = st.selectbox('Bulan', list(MONTH_MAP.keys()))
    with c4:
        moda = st.selectbox(
            'Moda', ['Transportasi Udara', 'Transportasi Laut']
        )

    if st.button('Generate Laporan'):
        df_curr, df_prev, df_cum_curr, df_cum_prev, prev_bln, prev_thn = (
            get_comparison_data(prov, thn, bln, moda)
        )
